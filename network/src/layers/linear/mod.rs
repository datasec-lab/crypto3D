...
use arithmetic::{fixed_point::*, fp_64::Fp64Parameters, FpParameters, PrimeField};
use primitives::AdditiveShare;
...
pub enum LinearLayerInfo<F, C> {
    Conv3d {
        kernel:  (usize, usize, usize, usize, usize),
        padding: Padding,
        stride:  usize,
    },
    FullyConnected,
...
impl<F, C> LinearLayer<F, C> {
    ...
    pub fn input_dimensions(&self) -> (usize, usize, usize, usize, usize) {
        self.dimensions().input_dimensions()
    }

    pub fn output_dimensions(&self) -> (usize, usize, usize, usize, usize) {
        self.dimensions().output_dimensions()
    }


    // (input_dims, output_dims, kernel_dims)
    //  MAKR WOULD BE PROBLEM ///
    ...
    pub fn all_dimensions(
        ...
        let kernel_dims = match self {
            Conv3d { dims: _, params: p } => p.kernel.dim(),
    ...
    {
        match self {
            Conv3d { dims: _, params: p } => p.kernel.to_repr(),
    ...
    pub fn eval_method(&self) -> crate::EvalMethod {
        match self {
            Conv3d { dims: _, params } => params.eval_method,
    ...
    {
        match self {
            Conv3d { dims: _, params: p } => {
                p.conv3d_naive(input, output);
            },
        ...
        C: Copy + One + std::fmt::Debug,
    {
        match self {
            LinearLayerInfo::AvgPool {
                ...
                let params = AvgPoolParams::new(*pool_d, *pool_h, *pool_w, *stride, *normalizer);
