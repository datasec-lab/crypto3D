...
where
    ...
    C: Copy,
{
    pub fn new(pool_d: usize, pool_h: usize, pool_w: usize, stride: usize, normalizer: C) -> Self {
        Self {
            pool_d,
            pool_h,
            pool_w,
    ...
    pub fn calculate_output_size(
        ...
        // assert_eq!(in_depth % (self.pool_d), 0);
        // assert_eq!(in_height % (self.pool_h), 0);
        // assert_eq!(in_width % (self.pool_w), 0);

        
        let out_depth = (in_depth - self.pool_d) / self.stride + 1;
    ...
    pub fn avg_pool_naive(&self, input: &Input<F>, out: &mut Output<F>) {
        ...
        for b_i in 0..batch_size {
            for (out_k, k) in (0..in_depth).step_by(self.stride).enumerate() {
                for (out_i, i) in (0..in_height).step_by(self.stride).enumerate() {
                    for (out_j, j) in (0..in_width).step_by(self.stride).enumerate() {
                        for chan in 0..in_channels {
                            let mut sum = F::zero();
                            for k_k in 0..self.pool_d {
                                for k_i in 0..self.pool_h {
                                    for k_j in 0..self.pool_w {
                                        sum += input[(b_i, chan, (k + k_k), (i + k_i), (j + k_j))];
                                    }
                                    
                                }

                            }
                            out[(b_i, chan, out_k, out_i, out_j)] = sum * self.normalizer;
                        }
                    }
                }
            }
