    ...
    C: Copy + Into<F>,
{
    ...
    pub fn calculate_output_size(
        ...
        let (num, w_channels, w_depth, w_height, w_width) = self.weights.dim();
        assert_eq!(w_depth, in_depth);
    ...
    pub fn fully_connected_naive(&self, input: &Input<F>, out: &mut Output<F>) {
        ...
        // need to update the depth
        let k = in_depth * in_height * in_width;
        let in_slice = s![0..batch_size, 0..in_channels, 0..in_depth, 0..in_height, 0..in_width];
        let w_slice = s![0..num, 0..w_channels, 0..w_height, 0..in_depth, 0..w_width];
        let inp = input
            .slice(&in_slice)
