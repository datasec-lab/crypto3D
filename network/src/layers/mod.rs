...
pub struct LayerDims {
    /// Dimension of the input to a layer: `(batch_size, channels, depth, height,
    /// width)`
    pub input_dims:  (usize, usize, usize, usize, usize),
    /// Dimension of the output of a layer: `(batch_size, channels, depth, height,
    /// width)`
    pub output_dims: (usize, usize, usize, usize, usize),
}
...
impl<'a, F, C: Clone> From<&'a Layer<F, C>> for LayerInfo<F, C> {
    fn from(other: &'a Layer<F, C>) -> Self {
        match other {
            LL(LinearLayer::Conv3d { dims, params }) => LayerInfo::LL(
                *dims,
