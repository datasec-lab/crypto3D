...
use layers::{
    Layer, LayerInfo,
    LinearLayer::{Conv3d, FullyConnected},
};
use npy::NpyData;
    ...
    pub fn from_numpy(&mut self, weights_path: &str) -> Result<(), ndarray::ShapeError> {
        ...
        for layer in self.layers.iter_mut() {
            let (kernel, bias) = match layer {
                Layer::LL(Conv3d { dims: _, params }) => (&params.kernel, &params.bias),
            ...
            match layer {
                Layer::LL(Conv3d { dims: _, params }) => params.kernel.iter_mut(),
            ...
            match layer {
                Layer::LL(Conv3d { dims: _, params }) => params.bias.iter_mut(),
