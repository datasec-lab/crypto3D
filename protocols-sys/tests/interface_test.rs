...
fn test_convolution() {
    ...
    let input_dims = (1, 1, 28, 28, 28);
    let kernel_dims = (16, 1, 5, 5, 5);
    let stride = 1;
    ...
    let layer_params =
        Conv3dParams::<TenBitAS, _>::new(padding, stride, kernel.clone(), bias.clone());
    let pt_layer_params =
    ...
    let mut client_cg = SealClientCG::Conv3D(client_cg::Conv3D::new(
        &cfhe,
        &layer_info,
        input_dims,
        output_dims,
    ...
    let mut server_cg =
        SealServerCG::Conv3D(server_cg::Conv3D::new(&sfhe, &layer, &kernel.to_repr()));
...
fn test_fully_connected() {
    ...
    let input_dims = (1, 3, 32, 32, 32);
    let kernel_dims = (10, 3, 32, 32, 32);
