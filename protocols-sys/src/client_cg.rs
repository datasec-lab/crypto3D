...
use primitives::additive_share::AdditiveShare;
use network::{
...
impl<'a> SealClientCG<'a> {
    ...
    pub fn decrypt(&mut self, linear_ct: Vec<c_char>) {
        match self {
            Self::Conv3D(s) => s.decrypt(linear_ct),
    ...
    {
        match self {
            Self::Conv3D(s) => ClientCG::postprocess::<P>(s, linear_share),
            Self::FullyConnected(s) => ClientCG::postprocess::<P>(s, linear_share),
        };
    }
}

impl<'a> ClientCG for Conv3D<'a> {
    ...
    fn new<F, C>(
        ...
        layer_info: &LinearLayerInfo<F, C>,
        input_dims: (usize, usize, usize, usize, usize),
        _output_dims: (usize, usize, usize, usize, usize),
    ) -> Self {
        let (kernel, padding, stride) = match layer_info {
            LinearLayerInfo::Conv3d {
                kernel,
                padding,
                stride,
        ...
        for chan in 0..self.data.out_chans as usize {
            for row in 0..self.data.output_h as usize {
                for col in 0..self.data.output_w as usize {
                    ...
                    linear_share[[0, 0, chan, row, col]] = AdditiveShare::new(
...
impl<'a> ClientCG for FullyConnected<'a> {
    ...
    fn new<F, C>(
        ...
        let data = unsafe {
            fc_metadata(
                cfhe.encoder,
                (input_dims.1 * input_dims.2 * input_dims.3 * input_dims.4) as i32,
        ...
        P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
    {
        ...
        for row in 0..self.data.filter_h as usize {
            ...
            linear_share[[0, row, 0, 0, 0]] = AdditiveShare::new(FixedPoint::with_num_muls(
