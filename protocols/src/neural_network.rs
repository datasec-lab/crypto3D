        ...
        for (i, layer) in neural_network.layers.iter().enumerate() {
            match layer {
                Layer::NLL(NonLinearLayer::ReLU(dims)) => {
                    let (b, c, d, h, w) = dims.input_dimensions();
                    num_relu += b * c * d * h * w;
                }
                Layer::NLL(NonLinearLayer::PolyApprox { dims, .. }) => {
                    let (b, c, d, h, w) = dims.input_dimensions();
                    num_approx += b * c * d * h * w;
                }
                Layer::LL(layer) => {
                    let randomizer = match &layer {
                        LinearLayer::Conv3d { .. } | LinearLayer::FullyConnected { .. } => {
                            let mut cg_handler = match &layer {
                                LinearLayer::Conv3d { .. } => SealServerCG::Conv3D(
                                    server_cg::Conv3D::new(&sfhe, layer, &layer.kernel_to_repr()),
                                ),
                        ...
                        LinearLayerInfo::Conv3d { .. } | LinearLayerInfo::FullyConnected => {
                            let mut cg_handler = match &linear_layer_info {
                                LinearLayerInfo::Conv3d { .. } => {
                                    SealClientCG::Conv3D(client_cg::Conv3D::new(
                                        &cfhe,
                                        linear_layer_info,
                                        input_dims,
                                        output_dims,
