...
use example::c3d::construct_c3d; 
use network::{ndarray::Array5, npy::NpyData};
    ...
    App::new("inference")
        ...
        .arg(
            Arg::with_name("layers")
                ...
                .long("layers")
                .takes_value(true)
                .help("Number of polynomial layers (0-10)")
                .required(true),
        )
        .get_matches()
}
...
fn main() {
    ...
    // Build network
    let mut network = construct_c3d(None, 1, layers, &mut rng);
    ...
    let image = Array5::from_shape_vec((1, 3, 112, 112, 112), image_vec).unwrap();

    example::inference::inference::run(network, architecture, image, class);
}