use ::network as nn;
use arithmetic::{fields::near_mersenne_64::F, FixedPoint, FixedPointParameters, Polynomial};
use utils::*;
use io_utils::{counting::CountingIO, imux::IMuxSync};
use nn::{
    layers::{
        average_pooling::AvgPoolParams,
        convolution::{Conv3dParams, Padding},
...
fn sample_conv_layer<R: RngCore + CryptoRng>(
    ...
    kernel_dims: (usize, usize, usize, usize, usize),
    stride: usize,
    padding: Padding,
    rng: &mut R,
) -> (
    ...
    let mut bias = Kernel::zeros((kernel_dims.0, 1, 1, 1, 1));
    kernel
        .iter_mut()
    ...
    let layer_params = match vs {
        Some(vs) => Conv3dParams::<TenBitAS, _>::new_with_gpu(
            vs,
            padding,
            stride,
            kernel.clone(),
        ...
        None => Conv3dParams::<TenBitAS, _>::new(padding, stride, kernel.clone(), bias.clone()),
    };
    ...
    let layer = LinearLayer::Conv3d {
        dims: layer_dims,
        params: layer_params,
    };

    let pt_layer_params =
        Conv3dParams::<TenBitExpFP, _>::new(padding, stride, kernel.clone(), bias.clone());
    let pt_layer = LinearLayer::Conv3d {
        dims: layer_dims,
...
fn sample_fc_layer<R: RngCore + CryptoRng>(
    ...
    let weight_dims = (out_chn, input_dims.1, input_dims.2, input_dims.3, input_dims.4);
    let mut weights = Kernel::zeros(weight_dims);
    weights
        .iter_mut()
        .for_each(|w_i| *w_i = generate_random_number(rng).1);

    let bias_dims = (out_chn, 1, 1, 1, 1);
...
fn sample_avg_pool_layer(
    input_dims: (usize, usize, usize, usize, usize),
    (pool_d, pool_h, pool_w): (usize, usize, usize),
    stride: usize,
) -> LinearLayer<TenBitAS, TenBitExpFP> {
    let size = (pool_h * pool_w) as f64;
    let avg_pool_params = AvgPoolParams::new(pool_d, pool_h, pool_w, stride, TenBitExpFP::from(1.0 / size));
